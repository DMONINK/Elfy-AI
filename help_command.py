...
information and, unlike every other command's response, never
auto-delete — help is meant to stick around for people to read.

Also home to "mhelp", the owner-only counterpart: everything /help shows,
plus the owner-only commands (restart/status/memory-lookup). mhelp is
deliberately tag-only, with no slash-command or embed version — see
build_mhelp_text()'s docstring.
"""
import re
...
# Single source of truth for the public help content: (section heading,
# body). Rendered as embed fields for /help and as plain-text lines for
# the "@Elfy help" mention trigger. Must NOT list bot restart, bot
# status, or the memory-lookup command — see OWNER_ONLY_SECTIONS below
# for those.
HELP_SECTIONS = [
    (
        "Chat with me",
...
# Owner-only additions layered on top of HELP_SECTIONS for mhelp. Kept
# tag-only on purpose — see build_mhelp_text().
OWNER_ONLY_SECTIONS = [
    (
        "Owner-only (tag-only, not slash — by design)",
...
def build_help_embed() -> discord.Embed:
    """Embed version of the public help text — used by the /help slash
    command (slash command responses are always embeds, see commands.py)."""
...
def build_help_text() -> str:
    """Plain-text version of the public help — used by the "@Elfy help"
    mention trigger (mention-triggered replies are plain text, never
    embeds, matching the rest of message_handler.py's non-slash
    responses)."""
    lines = [f"**{HELP_TITLE}**"]
    for name, value in HELP_SECTIONS:
...
def build_mhelp_text() -> str:
    """Plain-text mhelp content: everything /help shows, plus the
    owner-only commands. Deliberately tag-only, with no slash-command or
    embed counterpart: registering owner commands as slash commands would
    make them discoverable/visible in Discord's UI to every member, which
    defeats the point of restricting them. See mention_commands.py for
    the owner-ID gate on who can actually trigger this."""
    lines = [f"**{MHELP_TITLE}**"]
    for name, value in HELP_SECTIONS + OWNER_ONLY_SECTIONS:
        lines.append(f"**{name}** — {value}")
    return "\n".join(lines)
...
async def send_help_mention(message: Message) -> None:
    ...
    Deliberately does NOT auto-delete, unlike every other command's
    response — help is meant to stick around for people to actually read,
    not vanish after 10 seconds."""
    try:
        await message.reply(build_help_text(), mention_author=False)
    except Exception as e:
        print(f"[help] Mention reply failed: {e}")


async def send_mhelp_mention(message: Message) -> None:
    """Reply to a '@Elfy mhelp' mention with the owner help summary.
    Same no-auto-delete treatment as send_help_mention. Callers must
    already have verified the sender is an owner — see
    mention_commands.py — this function doesn't re-check."""
    try:
        await message.reply(build_mhelp_text(), mention_author=False)
    except Exception as e:
        print(f"[mhelp] Mention reply failed: {e}")


async def send_help_slash(interaction: Interaction) -> None:
    """Respond to /help with the embed version. Deliberately does NOT
    auto-delete (previously self-deleted after 5 seconds — help is meant
    to stick around now, unlike every other command's response)."""
